...
if os.name == "nt":  # pragma: no cover - imported only on Windows
    import msvcrt
else:  # pragma: no cover - imported only on POSIX
    msvcrt = None
...
def _lock_file_exclusive(lock_file) -> None:
    """Acquire an exclusive file lock on the current platform."""
...
def _unlock_file(lock_file) -> None:
    """Release an exclusive file lock on the current platform."""
    if fcntl is not None:
