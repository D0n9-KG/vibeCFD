...
**Example 4: "评估这个潜艇几何并准备 CFD 执行"**
→ Turn 1: Launch 3 subagents in parallel:
- `submarine-task-intelligence`: Match the uploaded geometry and task to candidate submarine cases
- `submarine-geometry-preflight`: Inspect `.x_t` / `.stl` readiness and preprocessing risks
- `submarine-solver-dispatch`: Prepare the next-stage solver handoff assumptions
→ Turn 2: If needed, launch `submarine-result-reporting`
→ Final turn: Synthesize the full submarine run status for the user
...
def get_subagent_skill_routing_prompt_section() -> str:
    """Describe recommended skill routing for specialized subagents.

    This gives the lead agent a graph-assisted starting point before it calls
    `task`, so it can explicitly pass `target_skills` when a narrower subset is
    appropriate for the delegated job.
    """
    submarine_subagents = [
    ...
    for subagent_name in submarine_subagents:
    ...
    routing_text = "\n".join(routing_lines)
    return f"""<subagent_skill_routing>
When delegating with the `task` tool, use the local skill graph as a routing aid.
Prefer passing an explicit `target_skills` list when the delegated task only needs
a narrow slice of the enabled skill pool. If `target_skills` is omitted, the
subagent will receive the normal enabled skill pool, so use the graph summary
below to make an explicit choice whenever task scope is specialized.

Recommended starting skill groups:
{routing_text}
