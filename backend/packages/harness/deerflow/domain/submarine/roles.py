...
ROLE_BOUNDARIES = [
    SubmarineRoleBoundary(
        ...
        title="案例检索与任务理解",
        responsibility=("将用户目标、工况和上传几何映射到最接近的潜艇 CFD 案例模板，并说明为什么选择它。"),
    ...
    SubmarineRoleBoundary(
        ...
        title="几何检查与预处理",
        responsibility=("检查 STL 输入、估计尺度、识别几何家族，并为后续求解准备可追踪的前处理产物。"),
    ...
    SubmarineRoleBoundary(
        ...
        title="求解调度",
        responsibility=("把已确认的几何和案例模板映射到受控的求解任务，后续可自然接入 OpenFOAM 或其他专业求解器。"),
    ...
    SubmarineRoleBoundary(
        ...
        title="科学研究变体规划",
        responsibility=("围绕 baseline run 规划网格无关性、域敏感性和时间步敏感性等研究变体，让研究证据链不只停留在单次求解。"),
    ...
    SubmarineRoleBoundary(
        ...
        title="实验对比整理",
        responsibility=("把 baseline 与研究变体 run 整理成结构化 experiment manifest 和 compare summary，让差异分析可追踪、可复核。"),
    ...
    SubmarineRoleBoundary(
        ...
        title="科学核验",
        responsibility=("依据 verification requirement、study evidence 和 benchmark 情况，判断当前 run 是否具备更强的科研结论资格。"),
    ...
    SubmarineRoleBoundary(
        ...
        title="结果整理与报告生成",
        responsibility=("把几何检查、求解日志和后处理结果整理成中文报告、摘要和可展示 artifact。"),
    ...
    SubmarineRoleBoundary(
        ...
        title="科学跟进",
        responsibility=("根据 scientific remediation handoff 决定是否继续派发研究变体、刷新报告或记录人工后续动作，让科研闭环可以自然迭代。"),
