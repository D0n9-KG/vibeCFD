...
def _is_custom_variant_record(record: Mapping[str, Any]) -> bool:
    ...
    return run_role == "custom_variant" or variant_origin == "custom_variant" or run_id.startswith("custom:")
...
def build_experiment_linkage_assessment(
    *,
    ...
    registered_custom_variant_run_ids = _registered_custom_variant_run_ids(experiment_manifest_mapping)
    ...
    if manifest_baseline_run_id and compare_baseline_run_id and manifest_baseline_run_id != compare_baseline_run_id:
        linkage_issues.append("Experiment registry baseline run id does not match compare summary baseline run id.")
    ...
    if manifest_experiment_id and compare_experiment_id and manifest_experiment_id != compare_experiment_id:
        linkage_issues.append("Experiment registry experiment_id does not match compare summary experiment_id.")

    run_record_ids = {_as_string(record.get("run_id")) for record in run_records if _as_string(record.get("run_id"))}
    if manifest_baseline_run_id and manifest_baseline_run_id not in run_record_ids:
        linkage_issues.append("Experiment registry is missing the declared baseline run record.")

    missing_variant_run_records = [run_id for run_id in expected_variant_run_ids if run_id not in recorded_variant_run_ids]
    linkage_issues.extend(f"Planned scientific variant is missing from experiment run records: {run_id}." for run_id in missing_variant_run_records)

    missing_compare_entries = [run_id for run_id in expected_variant_run_ids if run_id not in compared_variant_run_ids]
    linkage_issues.extend(f"Planned scientific variant is missing from experiment compare entry coverage: {run_id}." for run_id in missing_compare_entries)

    orphan_compare_entries = [run_id for run_id in compared_variant_run_ids if run_id not in recorded_variant_run_ids]
    linkage_issues.extend(f"Experiment compare entry has no matching run record: {run_id}." for run_id in orphan_compare_entries)

    additional_variant_run_ids = [run_id for run_id in recorded_variant_run_ids if run_id not in expected_variant_run_ids and run_id not in registered_custom_variant_run_ids]
    linkage_issues.extend(f"Experiment registry contains an unrecognized variant run record: {run_id}." for run_id in additional_variant_run_ids)
    variant_run_status_by_id = {_as_string(record.get("run_id")): _as_string(record.get("execution_status")) for record in run_records if _as_string(record.get("run_id")) and _as_string(record.get("run_id")) != "baseline"}
    compare_status_by_id = {
        _as_string(comparison.get("candidate_run_id")): _as_string(comparison.get("compare_status"))
        for comparison in comparisons
        if _as_string(comparison.get("candidate_run_id")) and _as_string(comparison.get("candidate_run_id")) != "baseline"
    }
    baseline_status = next(
        (_as_string(record.get("execution_status")) for record in run_records if _as_string(record.get("run_id")) == manifest_baseline_run_id),
        "",
    )
    ...
    planned_variant_run_ids = [run_id for run_id in expected_variant_run_ids if variant_run_status_by_id.get(run_id) == "planned"]
    in_progress_variant_run_ids = [run_id for run_id in expected_variant_run_ids if variant_run_status_by_id.get(run_id) == "in_progress"]
    completed_variant_run_ids = [run_id for run_id in expected_variant_run_ids if variant_run_status_by_id.get(run_id) == "completed"]
    blocked_variant_run_ids = [run_id for run_id in expected_variant_run_ids if variant_run_status_by_id.get(run_id) == "blocked"]
    planned_compare_variant_run_ids = [run_id for run_id in expected_variant_run_ids if compare_status_by_id.get(run_id) == "planned"]
    completed_compare_variant_run_ids = [run_id for run_id in expected_variant_run_ids if compare_status_by_id.get(run_id) == "completed"]
    blocked_compare_variant_run_ids = [run_id for run_id in expected_variant_run_ids if compare_status_by_id.get(run_id) == "blocked"]
    missing_metrics_variant_run_ids = [run_id for run_id in expected_variant_run_ids if compare_status_by_id.get(run_id) == "missing_metrics"]
    planned_custom_variant_run_ids = [run_id for run_id in registered_custom_variant_run_ids if variant_run_status_by_id.get(run_id) in {"planned", "in_progress"}]
    completed_custom_variant_run_ids = [run_id for run_id in registered_custom_variant_run_ids if variant_run_status_by_id.get(run_id) == "completed"]
    missing_custom_compare_entry_ids = [run_id for run_id in registered_custom_variant_run_ids if run_id not in compared_variant_run_ids]

    workflow_status = "planned"
    expected_run_statuses = [variant_run_status_by_id.get(run_id) for run_id in expected_variant_run_ids]
    expected_compare_statuses = [compare_status_by_id.get(run_id) for run_id in expected_variant_run_ids]
    registered_custom_run_statuses = [variant_run_status_by_id.get(run_id) for run_id in registered_custom_variant_run_ids]
    registered_custom_compare_statuses = [compare_status_by_id.get(run_id) for run_id in registered_custom_variant_run_ids]
    if blocked_variant_run_ids or blocked_compare_variant_run_ids or any(status == "blocked" for status in [*registered_custom_run_statuses, *registered_custom_compare_statuses]):
