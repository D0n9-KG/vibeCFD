...
OPENFOAM_SANDBOX_IMAGE="deer-flow-openfoam-sandbox:latest"
OPENFOAM_SANDBOX_DOCKERFILE="$PROJECT_ROOT/docker/openfoam-sandbox/Dockerfile"
OPENFOAM_SANDBOX_CONTEXT="$PROJECT_ROOT/docker/openfoam-sandbox"
...
build_local_sandbox_image() {
    ...
    if [ "$image" != "$OPENFOAM_SANDBOX_IMAGE" ]; then
        return 1
    fi

    if [ ! -f "$OPENFOAM_SANDBOX_DOCKERFILE" ]; then
        return 1
    fi

    echo -e "${BLUE}Building local OpenFOAM sandbox image: $image ...${NC}"
    echo ""

    docker build \
        -t "$image" \
        -f "$OPENFOAM_SANDBOX_DOCKERFILE" \
        "$OPENFOAM_SANDBOX_CONTEXT"
}
...
init() {
    ...
    if ! docker images --format '{{.Repository}}:{{.Tag}}' | grep -q "^${sandbox_image}$"; then
        if build_local_sandbox_image "$sandbox_image" 2>&1; then
            echo ""
        ...
        if ! docker pull "$sandbox_image" 2>&1; then
            ...
            echo "  4. The image is a local custom image that must be built first"
            echo ""
            echo -e "${GREEN}The Docker development environment can still be started after the image issue is resolved.${NC}"
            echo "If you need AIO sandbox (container-based execution):"
            echo "  - Ensure you have network access to the registry"
            echo "  - Or build the configured local sandbox image first"
            if [ "$sandbox_image" = "$OPENFOAM_SANDBOX_IMAGE" ]; then
                echo "  - Build command: docker build -t $sandbox_image -f $OPENFOAM_SANDBOX_DOCKERFILE $OPENFOAM_SANDBOX_CONTEXT"
            else
