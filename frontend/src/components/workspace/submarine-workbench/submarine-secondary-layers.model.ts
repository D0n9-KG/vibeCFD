...
function hasStudySignals(detail: SubmarineDetailModel): boolean {
  ...
  return [
    ...
    board.lineageModeLabel,
    board.compareTargetRunIds.length > 0,
    board.followupSourceRunId,
...
function hasOperatorSignals(detail: SubmarineDetailModel): boolean {
  ...
  return [
    ...
    board.contract.revisionLabel,
    board.contract.iterationModeLabel,
    board.contract.revisionSummary,
    board.contract.capabilityGapCount > 0,
    board.contract.unresolvedDecisionCount > 0,
    board.contract.evidenceExpectationCount > 0,
    board.contract.deliveryDeliveredCount > 0,
    board.contract.deliveryPlannedCount > 0,
    board.contract.deliveryBlockedCount > 0,
