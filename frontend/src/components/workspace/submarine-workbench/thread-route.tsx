...
function buildSubmarineThreadPath({
  threadId,
  isMock,
}: {
  threadId: string;
...
type SubmarineNegotiationRailBodyProps = {
  ...
  isNewThread: boolean;
  isUploading: boolean;
  inputStatus: "error" | "streaming" | "ready";
...
export function SubmarineThreadRoute({
  routeThreadId,
}: SubmarineThreadRouteProps) {
  ...
  const handleSubmitVisibleAction = useCallback(
    (text: string) => {
      ...
      void sendMessage(threadId, { text: trimmed, files: [] });
      window.setTimeout(() => {
        focusChatRail();
      }, 50);
    },
    [focusChatRail, sendMessage, threadId],
  );
  ...
  const chatRailErrorMessage = thread.error
    ...
    : null;
  const inputStatus = deriveThreadInputStatus({
    hasError: Boolean(chatRailErrorMessage),
    isLoading: thread.isLoading,
    latestRunStatus: streamMeta.latestRunStatus,
  });
          ...
          <main className="min-h-0 flex-1 overflow-hidden">
            <div className="mx-auto flex h-full w-full max-w-[1720px] flex-col px-4 py-4 md:px-5">
              <SubmarineAgenticWorkbench
                ...
                onSubmitVisibleAction={handleSubmitVisibleAction}
                visibleActionDisabled={
