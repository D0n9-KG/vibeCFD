...
function pickImageArtifacts(run: RunSummary | null): ArtifactItem[] {
  return (run?.artifacts ?? []).filter((artifact) => artifact.category === "image");
}
...
function renderViewportContent(
  ...
  if (artifact) {
    return <img className="viewport-image" src={artifact.url} alt={artifact.label} />;
  }
...
export function GraphicsWorkspace({ run }: GraphicsWorkspaceProps) {
  ...
  const primaryImage = imageArtifacts[0];
  const secondaryImage = imageArtifacts[1];
        ...
        <div className="workspace-grid">
          <article className="viewport viewport--primary">
            <header className="viewport__header">
              <span>主视窗</span>
              <strong>{primaryImage?.label ?? "几何 / 结果主图"}</strong>
            </header>
